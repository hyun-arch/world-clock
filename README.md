...
시계 가젯입니다. **웹 · 데스크톱 가젯(Windows) · 크롬 익스텐션** 세 가지 형태로 제공됩니다.

## 🌐 지금 바로 열기 (설치 불필요, 어느 컴퓨터에서나)
### 👉 **https://hyun-arch.github.io/world-clock/**

- 🖥 **데스크톱 설치 파일(.exe)**: [Releases에서 다운로드](https://github.com/hyun-arch/world-clock/releases/latest) — 항상 위에 뜨는 가젯 + 시작 시 자동 실행
- 🧩 **크롬 익스텐션**: 이 저장소를 내려받아 `chrome://extensions` → 개발자 모드 → "압축해제된 확장 로드" → `chrome-extension` 폴더
